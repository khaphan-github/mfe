...
function getWindow(element: HTMLBodyElement | null): HTMLDivElement {
  return <HTMLDivElement>(element as any).querySelector('ngb-typeahead-window.dropdown-menu');
}
...
function expectWindowResults(element: any, expectedResults: string[]) {
...
describe('ngb-typeahead', () => {
  ...
  describe('selectOnExact set to true', () => {
    ...
    it('should select the only existing result when it matches the user input', fakeAsync(() => {
      ...
      fixture.detectChanges();
      expect(getWindow(compiled as any)).toBeNull();
      expect(inputEl.value).toBe('one more');
    }));
  ...
  describe('selectOnExact set to true and editable set to false', () => {
    ...
    it('should select the only existing result when it matches the user input', fakeAsync(() => {
      ...
      fixture.detectChanges();
      expect(getWindow(compiled as any)).toBeNull();
      expect(((fixture.componentInstance.form.controls as any) as any).control.value).toBe('one more');
      expect((fixture.componentInstance.form.controls as any).control.valid).toBeTrue();
    }));
    ...
    it('should not select the only existing result when it doesn`t match the user input', fakeAsync(() => {
      ...
      fixture.detectChanges();
      expectWindowResults(compiled, ['+one more']);
      expect((fixture.componentInstance.form.controls as any).control.value).toBeUndefined();
      expect((fixture.componentInstance.form.controls as any).control.valid).toBeFalse();
    }));
  });
});
...
class TestComponent {
  ...
  uppercaseFormatter = (s: any) => `${s}`.toUpperCase();
