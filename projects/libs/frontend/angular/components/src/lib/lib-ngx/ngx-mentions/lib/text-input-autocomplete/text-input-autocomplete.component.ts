...
export class TextInputAutocompleteComponent implements OnChanges, OnInit, OnDestroy {
  ...
  /**
   * @author kha
   * Ẩn menu khi click ngoài component
   */
  @HostListener("document:click")
  clickedOut = () => this.hideMenu();

  /**
  * @author kha
  * Bấm ESC thì hide menu
  */
  @HostListener('document:keydown.escape', ['$event'])
  onEscapeKey = (event: KeyboardEvent) => this.hideMenu();
  ...
  ngOnInit() {
    ...
    /**
   * @author chungnguyen
   * Dùng bắt sự kiện scroll của màn hình
   */
  ...
  onBeforeInput(event: any): void {
    ...
    if (key === matchMention?.triggerCharacter && precedingCharValid(precedingChar)) {
      (this._matchMention as any) = matchMention;
      this.showMenu();
      return;
    }
  ...
  /**
   * @author chungnguyen
   * Dùng bắt sự kiện scroll của màn hình đểu thiết lập vị trí menu
   */
  onScroll() {
    ...
    setTimeout(() => {
      if (this.dropdownMenu && this.dropdownMenu.nativeElement) {
        ...
        const ulElement = document.getElementById('template-menu-mention');
        const height = ulElement ? ulElement.offsetHeight : 0;

        /**
         * @author chungnguyen
         * Xử lý để auto vị trí của menu trên hay dưới input, textarea
         */
        if (window.innerHeight - bounds.bottom < height) {
          let top = - height;
          (this.menuCtrl as any).position.top = top;
        }
        ...
        if (bounds.top < height) {
          ...
          top = Math.min(top, this.textInputElement.clientHeight - 20) + lineHeight;
          (this.menuCtrl as any).position.top = top;
        }
      }
    }, 0);
    ...
    setTimeout(() => {
      if (this.dropdownMenu && this.dropdownMenu.nativeElement) {
        ...
        if (bounds.left + bounds.width + 25 > window.innerWidth) {
          ...
          (this.menuCtrl as any).position.left = left;
  ...
  getChoiceLabel(triggerCharacter: string, choice: any): string {
    ...
    return (this.mentionsConfig as any).find((item: { triggerCharacter: string; }) => item.triggerCharacter === triggerCharacter).getChoiceLabel(choice);
  }
